...
XilerMo tracks a deployable `wrangler.jsonc` and provides a one-click deployment entry. For a manual deployment, create the resources and replace the example values with your own before running the deploy commands.
...
Replace the account-specific example values in `wrangler.jsonc`: write the generated D1 `database_id` into it and set `FLAREMO_PUBLIC_URL` to your public origin. The bucket and Vectorize index names can stay as the suggested defaults.
